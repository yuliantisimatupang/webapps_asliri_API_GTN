...
if (WebUI.verifyElementPresent(findTestObject('Web/CWS/Verify/Checkbox Asli Fast'), 2, FailureHandling.STOP_ON_FAILURE)) {
    'go to ASLI FAST'
    gotoaslifast()
} else {
    'close browser'
...
def gotoaslifast() {
    ...
    'click asli fast'
    WebUI.check(findTestObject('Web/CWS/Verify/Checkbox Asli Fast'))

    'verify element checked'
    WebUI.verifyElementChecked(findTestObject('Web/CWS/Verify/Checkbox Asli Fast'), 2)
