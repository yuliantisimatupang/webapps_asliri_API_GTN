...
for (int excelRow : (1..getLastRow)) {
	...
	sk_date = data.getValue('sk_date', excelRow)
}


WebUI.callTestCase(findTestCase('CW/Verify/AHU/SK Company (Company Verification)/00 - Open Company Verification'), [:], FailureHandling.STOP_ON_FAILURE)
...
def inputform() {
	...
	WebUI.verifyElementPresent(findTestObject('Web/CWS/AHU/SK Company/input_decreedate'), 1)
	...
	WebUI.setText(findTestObject('Web/CWS/AHU/SK Company/input_decreedate'), sk_date)

	'button submit appear'
	WebUI.verifyElementClickable(findTestObject('Web/CWS/AHU/SK Company/button_Submit'))
	WebUI.scrollToElement(findTestObject('Web/CWS/AHU/SK Company/button_Submit'), 1)
	
	def checkBalance1 = CustomKeywords.'abstraction.customKeyword.getRemainingAccess'('/sk_company')

	'click button submit'
	WebUI.click(findTestObject('Web/CWS/AHU/SK Company/button_Submit'))
	
	def checkBalance2 = CustomKeywords.'abstraction.customKeyword.getRemainingAccess'('/sk_company')

	'wait result appear'
	WebUI.waitForElementPresent(findTestObject('Web/CWS/AHU/Result/result Company Verification'), 1)
	
	'confirm result'
	WebUI.verifyElementPresent(findTestObject('Web/CWS/AHU/Result/icon_company_name'), 1)
	WebUI.verifyMatch(WebUI.getText(findTestObject('Web/CWS/AHU/Result/span_company_name')), '*ND*G*LD S*L*S* G*D**', false)
	
	WebUI.verifyElementPresent(findTestObject('Web/CWS/AHU/Result/span_decree_no'), 1)
	WebUI.verifyMatch(WebUI.getText(findTestObject('Web/CWS/AHU/Result/span_decree_no')), '04/04', false)
	
	WebUI.verifyElementPresent(findTestObject('Web/CWS/AHU/Result/icon_decree_date'), 1)
	WebUI.verifyMatch(WebUI.getAttribute(findTestObject('Web/CWS/AHU/Result/icon_decree_date'), 'class'), 'mdi mdi-check match', false)
