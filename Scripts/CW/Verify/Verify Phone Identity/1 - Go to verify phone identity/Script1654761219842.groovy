...
WebUI.scrollToElement(findTestObject('Web/CWS/Verify/Checkbox Verify Phone Identity'), 1)

WebUI.delay(2)

if (WebUI.verifyElementPresent(findTestObject('Web/CWS/Verify/Checkbox Verify Phone Identity'), 2, FailureHandling.STOP_ON_FAILURE)) {
    'go to phone identify'
    gotophoneidentity()
} else {
    'close browser'
    WebUI.closeBrowser()
...
def gotophoneidentity() {
    'click checkbox verify phone identify'
    WebUI.check(findTestObject('Web/CWS/Verify/Checkbox Verify Phone Identity'))

    WebUI.delay(1)

    'verify element checked'
    WebUI.verifyElementChecked(findTestObject('Web/CWS/Verify/Checkbox Verify Phone Identity'), 2)
