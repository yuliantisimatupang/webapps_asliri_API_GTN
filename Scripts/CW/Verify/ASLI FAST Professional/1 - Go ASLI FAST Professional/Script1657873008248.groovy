...
if (WebUI.verifyElementPresent(findTestObject('Web/CWS/Verify/Checkbox ASLI FAST Professional'), 2, FailureHandling.STOP_ON_FAILURE)) {
    'go to ASLI FAST Profesional'
    gotoaslifastprofessional()
} else {
    'close browser'
    WebUI.closeBrowser()
}

def gotoaslifastprofessional() {
    WebUI.delay(1)

    'click asli fast professional'
    WebUI.check(findTestObject('Web/CWS/Verify/Checkbox ASLI FAST Professional'))

    'verify element checked'
    WebUI.verifyElementChecked(findTestObject('Web/CWS/Verify/Checkbox ASLI FAST Professional'), 2)
