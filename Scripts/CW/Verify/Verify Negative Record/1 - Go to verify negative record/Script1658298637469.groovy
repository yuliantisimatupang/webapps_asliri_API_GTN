...
WebUI.scrollToElement(findTestObject('Web/CWS/Verify/Checkbox Verify Negative Record'), 1)

WebUI.delay(2)

if (WebUI.verifyElementPresent(findTestObject('Web/CWS/Verify/Checkbox Verify Negative Record'), 2, FailureHandling.STOP_ON_FAILURE)) {
    'go to verify negative record'
    gotonegativerecord()
} else {
    'close browser'
    WebUI.closeBrowser()
...
def gotonegativerecord() {
    'click checkbox verify negative record'
    WebUI.check(findTestObject('Web/CWS/Verify/Checkbox Verify Negative Record'))

    WebUI.delay(1)

    'verify element checked'
    WebUI.verifyElementChecked(findTestObject('Web/CWS/Verify/Checkbox Verify Negative Record'), 2)

    WebUI.delay(1)

    'scroll to checkbox agreement'
