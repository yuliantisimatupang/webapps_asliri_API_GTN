...
if (WebUI.verifyElementPresent(findTestObject('Web/CWS/Verify/Checkbox Income Verification (NIK)'), 2, FailureHandling.STOP_ON_FAILURE)) {
    'go to incom verification nik'
    gotoincomeverificationnik()
} else {
    'close browser'
    WebUI.closeBrowser()
}

def gotoincomeverificationnik() {
    WebUI.delay(1)

    'click income verification nik'
    WebUI.check(findTestObject('Web/CWS/Verify/Checkbox Income Verification (NIK)'))

    'verify element checked'
    WebUI.verifyElementChecked(findTestObject('Web/CWS/Verify/Checkbox Income Verification (NIK)'), 2)
