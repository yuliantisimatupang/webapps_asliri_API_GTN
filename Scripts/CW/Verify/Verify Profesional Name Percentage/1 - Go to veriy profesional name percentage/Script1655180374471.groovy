...
'scroll to verify profesional name percentage'
WebUI.scrollToElement(findTestObject('Web/CWS/Verify/Checkbox Verify Profesional Name Percentage'), 2)

WebUI.delay(1)

if (WebUI.verifyElementPresent(findTestObject('Web/CWS/Verify/Checkbox Verify Income Grade'), 2, FailureHandling.STOP_ON_FAILURE)) {
    'go to verify profesional name percentage'
    gotonamepercentage()
} else {
    'close browser'
    WebUI.closeBrowser()
...
def gotonamepercentage() {
    WebUI.delay(1)

    'click verify profesional name percentage'
    WebUI.check(findTestObject('Web/CWS/Verify/Checkbox Verify Profesional Name Percentage'))
