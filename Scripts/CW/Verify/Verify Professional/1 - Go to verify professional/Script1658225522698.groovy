...
'scroll to verify professional'
WebUI.scrollToElement(findTestObject('Web/CWS/Verify/Checkbox Verify Professional'), 2)

WebUI.delay(1)

if (WebUI.verifyElementPresent(findTestObject('Web/CWS/Verify/Checkbox Verify Professional'), 2, FailureHandling.STOP_ON_FAILURE)) {
    'go to verify professional'
    gotoverifyprofessional()
} else {
    'close browser'
    WebUI.closeBrowser()
}

def gotoverifyprofessional() {
    WebUI.delay(1)

    'click verify professional'
    WebUI.check(findTestObject('Web/CWS/Verify/Checkbox Verify Professional'))

    'verify element checked'
    WebUI.verifyElementChecked(findTestObject('Web/CWS/Verify/Checkbox Verify Professional'), 2)
