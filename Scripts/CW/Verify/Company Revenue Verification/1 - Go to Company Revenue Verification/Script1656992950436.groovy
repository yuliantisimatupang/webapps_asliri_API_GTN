...
if (WebUI.verifyElementPresent(findTestObject('Web/CWS/Verify/Checkbox Company Revenue Verification'), 2, FailureHandling.STOP_ON_FAILURE)) {
    'go to company revenue verification'
    gotocompanyrevenue()
} else {
    'close browser'
    WebUI.closeBrowser()
...
def gotocompanyrevenue() {
    WebUI.delay(1)

    'click company revenue'
    WebUI.check(findTestObject('Web/CWS/Verify/Checkbox Company Revenue Verification'))

    WebUI.delay(1)

    'verify element checked'
    WebUI.verifyElementChecked(findTestObject('Web/CWS/Verify/Checkbox Company Revenue Verification'), 2)
