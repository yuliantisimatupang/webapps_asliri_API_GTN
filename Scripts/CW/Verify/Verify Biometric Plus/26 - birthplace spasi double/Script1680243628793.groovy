...
def inputform() {
	...
	WebUI.verifyMatch(FilePath2, 'C:\\fakepath\\ektp_yehuda.png', false)
	...
	if ((WebUI.verifyLessThanOrEqual(pdobStatus, 100.0)) && (WebUI.verifyGreaterThanOrEqual(pdobStatus, 50.0))) {
		assert true: pass
	}else {
		assert false: fail
	}
	...
	WebUI.verifyMatch(WebUI.getText(findTestObject('Web/CWS/Verify Biometric Plus/Result/span_address_hashout')), '*PT C*R TWR C*S*BL*NC* B L*NT** 11 N*.09', false)
